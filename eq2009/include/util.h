...
inline Item0 compute_ith_item(int seed, size_t leaf_index)
{
    ...
    *nonce_ptr = seed; // Already little-endian on x86
    ...
    ZcashEquihashHasher H;
    H.init_midstate(headernonce, sizeof(headernonce), dummy_nonce, /*n=*/200, /*k=*/9);
    ...
    Item0 item;
    const size_t offset = second ? 25 : 0;
    std::memcpy(item.XOR, out + offset, 25);
    return item;
}
...
{
    static constexpr size_t xor_len = sizeof(Layer_Type::value_type::XOR);
    static constexpr uint32_t HALF = 1u << 20; // 1,048,576
    static constexpr uint32_t FULL = HALF * 2; // 2,097,152
...
inline void expand_solution_from_file(Solution &solution, IPDiskReader &reader, const IPDiskMeta &meta)
{
    Solution out;
    out.reserve(solution.size() * 2);
    for (size_t idx_ref : solution)
...
inline void expand_solutions_from_file(std::vector<Solution> &solutions, IPDiskReader &reader, const IPDiskMeta &meta)
{
    ...
    const int &seed, const std::vector<Solution> &solutions)
{
    ...
    for (size_t ci = 0; ci < total; ++ci)
    {
        ...
        if (g_verbose)
        {
            ...
            if (is_200)
            {
                ::printf(" ✓ VALID 200-bit (all zeros)\n");
                ::printf("    Indices: ");
    ...
    if (g_verbose)
    {
        ...
        std::cout << "  Valid solutions: " << valid200 << "\n";
        std::cout << "========================================================="
                     "======================\n";
    }
    return valid200;
