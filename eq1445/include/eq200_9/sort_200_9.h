...
inline constexpr std::size_t EQ2009_COLLISION_BITS = EquihashParams::kCollisionBitLength;
inline constexpr std::size_t EQ2009_FINAL_BITS = EquihashParams::kCollisionBitLength * 2;
...
{
    return get_key_bits<T, EQ2009_COLLISION_BITS>(item);
}

template <typename T>
inline auto get_final_key(const T &item)
{
    return get_key_bits<T, EQ2009_FINAL_BITS>(item);
}
...
{
    sort_layer_by_key<T, EQ2009_COLLISION_BITS>(layer);
}

template <typename T>
inline void sort_final(LayerVec<T> &layer)
{
    sort_layer_by_key<T, EQ2009_FINAL_BITS>(layer);
}
